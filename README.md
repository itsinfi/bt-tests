# Evaluierung einer Migration von Node.js zu Bun am Beispiel eines Full-Stack-JavaScript-Systems

Dieses Repository enthält alle Testskripte zur Messung der API- (1) bzw. UI-Performance (2).

Das Repository basiert auf dem Repository für API-Benchmarks [(Angermann, 2025a)](https://github.com/itsinfi/hausarbeit-web-dev-tests), das zu einer vorigen Hausarbeit erstellt wurde (Angermann, 2025b).

## 1 API-Performance

TODO:

## 2 UI-Performance

TODO:

## Quellen

<p style="text-indent: -2em; margin-left: 2em;">
Angermann, S. (2025a). <i>hausarbeit-web-dev-tests</i> [GitHub-Repository]. GitHub. <a>https://github.com/itsinfi/hausarbeit-web-dev-tests</a>
</p>
<p style="text-indent: -2em; margin-left: 2em;">
Angermann, S. (2025b). <i>Benchmarking von Jakarta EE und Node.js: Performance aktueller Backend-Technologien
im Vergleich </i>  [unveröffentlichte Hausarbeit]. IU Internationale Hochschule.
</p>